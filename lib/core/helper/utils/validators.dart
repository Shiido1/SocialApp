...
class Validators {
  ...
  static final _validCharacters = 'abcdefghijklmnopqrstuvwxyz';

  /// validates users input to a valid username
  static String? Function(String?)? validateUsername({String? error}) {
    return (String? value) {
      if (value!.isEmpty) {
        return error ?? 'Enter a valid username';
      }
      if (!value.contains(RegExp("@"))) {
        return error ?? 'Usernames must begin with @';
      }
      if (value.contains(RegExp(" "))) {
        return error ?? 'Spaces are not allowed';
      }
      if (value.length < 3 || value.length > 30) {
        return error ?? 'Username should be 3 to 30 character length';
      }
      return null;
    };
  }
  ...
  static String? Function(String?)? validateConfirmPassword(String password) {
    return (String? value) {
      ...
      if (password.isEmpty) {
        return 'Please enter a passwoxrd.';
      }
